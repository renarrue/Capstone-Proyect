# Versión 39.0 (MASTER: Base V29 + Exportación + Easter Eggs + Saludos)
...
# --- LIBRERÍAS PARA EXPORTAR (Feature Agregada) ---
...
# --- EASTER EGGS Y SALUDOS (Feature Agregada) ---
EASTER_EGGS = {
...
# --- FUNCIONES DE EXPORTACIÓN (Feature Agregada) ---
...
if st.session_state["authentication_status"] is True:
    ...
    # SALUDO DINÁMICO (Feature Agregada)
        ...
        for msg in st.session_state.messages:
            with st.chat_message(msg["role"]):
                ...
                if msg["role"] == "assistant" and msg["id"]:
                    ...
                    # FEEDBACK QUE NO DESAPARECE (FIX DE PERSISTENCIA)
            ...
            if sugerencia:
                ...
                with st.chat_message("assistant"):
                    with st.spinner(t["chat_thinking"]):
                        # Lógica de Easter Eggs también para chips
                        prompt_limpio = sugerencia.lower().strip().strip("?!.,")
                        if "buscar" in prompt_limpio:
                ...
                with st.spinner(t["chat_thinking"]):
                    if prompt_limpio in EASTER_EGGS:
                        ...
                        try:
                            ...
                            resp = "Lo siento, tuve un problema leyendo los documentos."
    ...
    with tab2:
        ...
        # --- Lógica para exportación de horario (Feature Agregada) ---
        @st.cache_data(ttl=60)
        def get_full_schedule_info(uid):
             # Necesitamos un join completo para exportar
             return supabase.table('registrations').select('*, sections(*, subjects(*))').eq('user_id', uid).execute().data
        
        schedule_info = get_full_schedule_info(user_id)
        
        if schedule_info:
             # Botones de Exportar
             c_exp1, c_exp2 = st.columns(2)
             sections_data = [r['sections'] for r in schedule_info]
             ...
             for item in schedule_info:
                 sec = item['sections']
                 subj = sec['subjects']
                 with st.expander(f"📘 {subj['name']} ({sec['section_code']})"):
                     c1, c2 = st.columns([4,1])
                     c1.write(f"{sec['day_of_week']} {sec['start_time'][:5]}-{sec['end_time'][:5]} | Prof: {sec['professor_name']}")
                     if c2.button(t["btn_drop"], key=f"del_{item['id']}", type="primary"):
